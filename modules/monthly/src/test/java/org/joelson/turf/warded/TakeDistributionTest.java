...
public class TakeDistributionTest {
    ...
    private static void createDistributionMap(String filename, Set<String> zoneNames, boolean printZones)
        ...
        int nextMaxVisitsCalc = distributionList.stream().filter(
                zoneTakeDistribution -> zoneTakeDistribution.getVisits() == currentCalc).mapToInt(
                ZoneTakeDistribution::getMonthlyVisits).filter(value -> value > 0).min().orElse(0);
        ...
        distributionList.forEach(zoneTakeDistribution -> {
            if (zoneTakeDistribution.getPercentage() < cut || zoneTakeDistribution.getVisits() < current) {
                below.add(zoneTakeDistribution);
                    ...
                    || zoneTakeDistribution.getVisits() < current + 5) {
...
//
    ...
    private static int findNextCurrent(List<ZoneTakeDistribution> distributionList) {
        ...
        distributionList.forEach(
                zoneTakeDistribution -> zoneCount[Math.min(51, zoneTakeDistribution.getVisits())] += 1);
        int last = 0;
        ...
        return distributionList.stream().filter(zoneTakeDistribution -> zoneTakeDistribution.getMonthlyVisits() > 0)
                .mapToInt(ZoneTakeDistribution::getVisits).min().orElseThrow();
    }
    ...
    private static void writeDistributionSet(KMLWriter out, Set<ZoneTakeDistribution> zones, String name) {
        ...
        zones.stream().sorted(Comparator.comparing(ZoneTakeDistribution::getVisits)
                .thenComparing(zoneTakeDistribution -> zoneTakeDistribution.getZone().getName())).forEach(
                zoneTakeDistribution -> writeZone(out, zoneTakeDistribution));
    }

    private static void writeZone(KMLWriter out, ZoneTakeDistribution zoneTakeDistribution) {
        out.writePlacemark(zoneTakeDistribution.toKMLPlacemarkNameString(), "",
                zoneTakeDistribution.getZone().getLongitude(), zoneTakeDistribution.getZone().getLatitude());
    }
    ...
    private static class ZoneTakeDistribution {
        ...
        public String toCSVPlacemarkNameString() {
            return String.format(Locale.ENGLISH, "%s - %d - %d - %.2f%%", zone.getName(), visits, monthlyVisits,
                    percentage);
        }
    }
//
