...
final class TableUtil {
    ...
    private static Container createFilterContainer(TableRowSorter<TableModel> sorter, String filterLabelText) {
        ...
        groupLayout.setHorizontalGroup(
                groupLayout.createSequentialGroup().addComponent(filterLabel).addComponent(filterField));
        groupLayout.setVerticalGroup(groupLayout.createParallelGroup(Alignment.BASELINE).addComponent(filterLabel)
    ...
    private static final class DurationRenderer extends DefaultTableCellRenderer {
        ...
        public void setValue(Object value) {
            if (value instanceof Duration duration) {
                ...
                if (days == 0) {
                    setText(String.format("%d:%02d:%02d", duration.toHours(), duration.toMinutesPart(),
                            duration.toSecondsPart()));
                } else if (days == 1) {
                    setText(String.format("1 day %02d:%02d:%02d", duration.toHoursPart(), duration.toMinutesPart(),
                            duration.toSecondsPart()));
                } else {
                    setText(String.format("%d days %02d:%02d:%02d", duration.toDays(), duration.toHoursPart(),
